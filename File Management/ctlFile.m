...
%Define elements
CTLgeometry(SimGroup,geometry,simulation);
...
function CTLgeometry(SimGroup,geometry,simulation)
global fid

if(~simulation.normalization && simulation.epsInputFile) %Check for epsilon input file in use on reflctance run)
    fprintf(fid, ['(set! epsilon-input-file "' SimGroup.dir '/' SimGroup.name '/epsilon.h5")\n\n']);
else
    fprintf(fid,'(set! geometry (list ');
    
    if(simulation.normalization)
        objectNum = simulation.geometryNormalizationLimit;
    else
